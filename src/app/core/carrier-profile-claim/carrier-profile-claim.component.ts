...
@Component({
  selector: 'app-carrier-profil-claim',
  templateUrl: './carrier-profile-claim.component.html',
  styleUrls: ['./carrier-profile-claim.component.css']
})

export class CarrierProfileClaim implements OnInit {
 ...
 public errorMesage:any;
  ...
  searchUserType(event: any) {
    ...
    if(searchStr && searchStr.length >= 4){
      ...
      this.errorMesage='The minimum length should be 4 characters'
  ...
  carrierSignUp(dotNumber:any){
    this.router.navigateByUrl('/carrier-sign-up?dotNumber='+dotNumber);
  }
}
