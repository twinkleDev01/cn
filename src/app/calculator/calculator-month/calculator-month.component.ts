...
@Component({
  selector: 'app-calculator-month',
  templateUrl: './calculator-month.component.html',
  styleUrls: ['./calculator-month.component.scss']
})
export class CalculatorMonthComponent implements OnInit {
  ...
  constructor(private alertService: AlertService, private cd: ChangeDetectorRef, public commonService: CommonService, public dialog: MatDialog, private formBuilder: FormBuilder, public activeRouter: ActivatedRoute, public jsonService: fakeJson, public router: Router) { }
  ...
  addItem(type: string) {
    let items;
    if (type == 'companyOffice') { items = this.companyOfficeForm.get('items') as FormArray }
    else if (type == 'truckExpenses') { items = this.truckExpensesForm.get('items') as FormArray }
    else if (type == 'trailerExpenses') { items = this.trailerExpenseForm.get('items') as FormArray }
    else if (type == 'driverExpenses') { items = this.driverExpenseForm.get('items') as FormArray }
    else if (type == 'onRoadExpenses') { items = this.onRoadExpenseForm.get('items') as FormArray }
    else if (type == 'miscellaneousExpenses') { items = this.miscellaneousForm.get('items') as FormArray }
  ...
  fetchAndFillData(id: any) {
    ...
    this.commonService.getList(APIparams).subscribe((value) => {
      this.calculatorData = value.response.filter((data: any) => data.type == 2)
  ...
  removeItem(index: number, arrayName: string, form: string) {
    let items;
    if (form == 'companyOffice') { items = this.companyOfficeForm.get('items') as FormArray }
    else if (form == 'truckExpenses') { items = this.truckExpensesForm.get('items') as FormArray }
    else if (form == 'trailerExpenses') { items = this.trailerExpenseForm.get('items') as FormArray }
    else if (form == 'driverExpenses') { items = this.driverExpenseForm.get('items') as FormArray }
    else if (form == 'onRoadExpenses') { items = this.onRoadExpenseForm.get('items') as FormArray }
    else if (form == 'miscellaneousExpenses') { items = this.miscellaneousForm.get('items') as FormArray }
    ...
    if (arrayName == 'companyOfficeInputs') {
      ...
      this.companyOfficeForm.value.items.forEach(element => {
        this.companyOfficeTotal = this.companyOfficeTotal + +element.perMonth;
      });
    }
    if (arrayName == 'truckExpensesInputs') {
      ...
      this.truckExpensesForm.value.items.forEach(element => {
        this.truckExpensesTotal = this.truckExpensesTotal + +element.perMonth;
      });
    }
    if (arrayName == 'trailerExpensesInputs') {
      ...
      this.trailerExpenseForm.value.items.forEach(element => {
        this.trailerExpensesTotal = this.trailerExpensesTotal + +element.perMonth;
      });
    }
    if (arrayName == 'driverExpensesInputs') {
      ...
      this.driverExpenseForm.value.items.forEach(element => {
        this.driverExpensesTotal = this.driverExpensesTotal + +element.perMonth;
      });
    }
    if (arrayName == 'onRoadExpensesInputs') {
      ...
      this.onRoadExpenseForm.value.items.forEach(element => {
        this.onRoadExpensesTotal = this.onRoadExpensesTotal + +element.perMonth;
      });
    }
    if (arrayName == 'miscellaneousInputs') {
      ...
      this.miscellaneousForm.value.items.forEach(element => {
        this.miscellaneousTotal = this.miscellaneousTotal + +element.perMonth;
      });
    }
  }
      ...
      this.companyOfficeForm.value.items.forEach((element, index) => {
        ...
        if (element.unit == 'mile' || element.unit == 'day') {
          ...
          if (item?.get('amount').value) {
            ...
            item?.get('perMonth').setValue(newPerTripValue);
            let newPerMileValue = +(item?.get('perMonth').value / this.totalMiles).toFixed(2)
            item?.get('perMile').setValue(newPerMileValue);
            this.companyOfficeTotal = this.companyOfficeTotal + +item?.get('perMonth').value;
          }
        }
        else {
          const item = items.at(index) as FormGroup;
      ...
      this.truckExpensesForm.value.items.forEach((element, index) => {
        ...
        if (element.unit == 'mile' || element.unit == 'day') {
          ...
          if (item?.get('amount').value) {
            ...
            let newPerMileValue = +(item?.get('perMonth').value / this.totalMiles).toFixed(2)
            item?.get('perMile').setValue(newPerMileValue);
            this.truckExpensesTotal = this.truckExpensesTotal + +item?.get('perMonth').value;
          }
        }
        else {
      ...
      this.trailerExpenseForm.value.items.forEach((element, index) => {
        ...
        if (element.unit == 'mile' || element.unit == 'day') {
          ...
          if (item?.get('amount').value) {
            ...
            let newPerMileValue = +(item?.get('perMonth').value / this.totalMiles).toFixed(2)
            item?.get('perMile').setValue(newPerMileValue);
            this.trailerExpensesTotal = this.trailerExpensesTotal + +item?.get('perMonth').value;
          }
        }
        else {
          const item = items.at(index) as FormGroup;
      ...
      this.driverExpenseForm.value.items.forEach((element, index) => {
        ...
        if (element.unit == 'mile' || element.unit == 'day') {
          ...
          if (item?.get('amount').value) {
            ...
            let newPerMileValue = +(item?.get('perMonth').value / this.totalMiles).toFixed(2)
            item?.get('perMile').setValue(newPerMileValue);
            this.driverExpensesTotal = this.driverExpensesTotal + +item?.get('perMonth').value;
          }
        }
        else {
          const item = items.at(index) as FormGroup;
      ...
      this.onRoadExpenseForm.value.items.forEach((element, index) => {
        ...
        if (element.unit == 'mile' || element.unit == 'day') {
          ...
          if (item?.get('amount').value) {
            ...
            let newPerMileValue = +(item?.get('perMonth').value / this.totalMiles).toFixed(2)
            item?.get('perMile').setValue(newPerMileValue);
            this.onRoadExpensesTotal = this.onRoadExpensesTotal + +item?.get('perMonth').value;
          }
        }
        else {
          const item = items.at(index) as FormGroup;
      ...
      this.miscellaneousForm.value.items.forEach((element, index) => {
        ...
        if (element.unit == 'mile' || element.unit == 'day') {
          ...
          if (item?.get('amount').value) {
            ...
            let newPerMileValue = +(item?.get('perMonth').value / this.totalMiles).toFixed(2)
            item?.get('perMile').setValue(newPerMileValue);
            this.miscellaneousTotal = this.miscellaneousTotal + +item?.get('perMonth').value;
          }
        }
        else {
          const item = items.at(index) as FormGroup;
  ...
  onAmountInput(itemIndex, form, fetchForm?) {
    ...
    if (form == 'truckExpensesForm') {
      ...
      else { items = this.truckExpensesForm.get('items') as FormArray; }
    }
    if (form == 'trailerExpenseForm') {
      if (fetchForm) {
        items = fetchForm;
      }
      else { items = this.trailerExpenseForm.get('items') as FormArray; }
    }
    if (form == 'driverExpenseForm') {
      if (fetchForm) {
        items = fetchForm;
      }
      else { items = this.driverExpenseForm.get('items') as FormArray; }
    }
    if (form == 'onRoadExpenseForm') {
      if (fetchForm) {
        items = fetchForm;
      }
      else { items = this.onRoadExpenseForm.get('items') as FormArray; }
    }
    if (form == 'miscellaneousForm') {
      if (fetchForm) {
        items = fetchForm;
      }
      else { items = this.miscellaneousForm.get('items') as FormArray; }
    }
  ...
  onUnitSelection(itemIndex: number, value: any, itemform, form) {
    let items
    if (form == 'companyOfficeForm') { items = this.companyOfficeForm.get('items') as FormArray; }
    if (form == 'truckExpensesForm') { items = this.truckExpensesForm.get('items') as FormArray; }
    if (form == 'trailerExpenseForm') { items = this.trailerExpenseForm.get('items') as FormArray; }
    if (form == 'driverExpenseForm') { items = this.driverExpenseForm.get('items') as FormArray; }
    if (form == 'onRoadExpenseForm') { items = this.onRoadExpenseForm.get('items') as FormArray; }
    if (form == 'miscellaneousForm') { items = this.miscellaneousForm.get('items') as FormArray; }
    const item = items.at(itemIndex) as FormGroup;
  ...
  setPerValuesPerTrip(itemIndex: number, perMileValue: number, perTripValue: number, form) {
    ...
    item?.get('perMonth').setValue(perTripValue);
    this.companyOfficeTotal = 0;
    this.truckExpensesTotal = 0;
    this.trailerExpensesTotal = 0;
    this.driverExpensesTotal = 0;
    ...
    this.miscellaneousTotal = 0;
    this.companyOfficeForm.value.items.forEach(element => {
      this.companyOfficeTotal = this.companyOfficeTotal + (isNaN(element.perMonth) ? 0 : +element.perMonth);
    });
    this.truckExpensesForm.value.items.forEach(element => {
      this.truckExpensesTotal = this.truckExpensesTotal + (isNaN(element.perMonth) ? 0 : +element.perMonth);
    });
    this.trailerExpenseForm.value.items.forEach(element => {
      this.trailerExpensesTotal = this.trailerExpensesTotal + (isNaN(element.perMonth) ? 0 : +element.perMonth);
    });
    this.driverExpenseForm.value.items.forEach(element => {
      this.driverExpensesTotal = this.driverExpensesTotal + (isNaN(element.perMonth) ? 0 : +element.perMonth);
    });
    this.onRoadExpenseForm.value.items.forEach(element => {
      this.onRoadExpensesTotal = this.onRoadExpensesTotal + (isNaN(element.perMonth) ? 0 : +element.perMonth);
    });
    this.miscellaneousForm.value.items.forEach(element => {
      this.miscellaneousTotal = this.miscellaneousTotal + (isNaN(element.perMonth) ? 0 : +element.perMonth);
    });
  ...
  onSubmit() {
    ...
    for (let i = 0; i < this.companyOfficeInputs.length; i++) {
      ...
      allFormsData.companyOfficeExpenses[key] = {
    ...
    for (let i = 0; i < this.truckExpensesInputs.length; i++) {
      ...
      allFormsData.truckExpenses[key] = {
    ...
    for (let i = 0; i < this.trailerExpensesInputs.length; i++) {
      ...
      allFormsData.trailerExpenses[key] = {
    ...
    for (let i = 0; i < this.driverExpensesInputs.length; i++) {
      ...
      allFormsData.driveExpenses[key] = {
    ...
    for (let i = 0; i < this.onRoadExpensesInputs.length; i++) {
      ...
      allFormsData.onRoadExpenses[key] = {
    ...
    for (let i = 0; i < this.miscellaneousInputs.length; i++) {
      ...
      allFormsData.miscellaneousExpenses[key] = {
    ...
    allFormsData.calculatorName = 'Mexico Route Calculator'
    allFormsData.otherTripDataPoints = {
      truckAverage: this.truckAverage,
      totalMiles: this.totalMiles,
      totalDays: this.totalDays,
      revenue: this.revenueAverage
    }
        ...
        if (ctx) {
          ...
          canvas.chart = new Chart(ctx, {
            ...
            data: {
              ...
              datasets: [{
                label: 'Price of Various Expenses',
  ...
  overallTotal(type) {
    let total = this.companyOfficeTotal + this.truckExpensesTotal + this.trailerExpensesTotal + this.driverExpensesTotal + this.miscellaneousTotal + this.onRoadExpensesTotal;

    if (type == 'trip') {
      return total.toFixed();
    }
    if (type == 'mile') {
      return (this.totalMiles ? (total / this.totalMiles).toFixed() : '-');
    }
  }
  ...
  postCalculator(data: any) {
    ...
    this.commonService.post(APIparams).subscribe((value) => {
      ...
      this.router.navigate(['./calculator/manage-cp-month'])
    })
  }
  ...
  payloadOfAllData() {
    ...
    allFormsData.expenses.calculatorName = 'Mexico Route Calculator'
