...
@Component({
  selector: 'app-non-carrier-lanes-regions',
  templateUrl: './non-carrier-lanes-regions.component.html',
  styleUrls: ['./non-carrier-lanes-regions.component.css']
})
export class NonCarrierLanesRegionsComponent implements OnInit {
  ...
  getCarriers() {
    ...
    this.commonService.getList(APIparams).subscribe((ServerResult) => {
      if (ServerResult.success === true) {
        ...
        this.selectedHead = ServerResult.response;
        
        console.log(this.selectedHead.headQuarter.cityName ,'this.selectedHead ')
  ...
  getRegionHeadQuarter(record: any) {
    ...
    this.selectRegionForm = this.formBuilder.group({
      preferredRegions: [''],
      headquarter: [this.selectedHead?.headQuarter?.cityId, this.selectedHead?.headQuarter?.cityId ? [Validators.required] : [],],
    });
  }
