// oct 2024 I have dymanic the api for broker and carrier
...
export class BrokerContactInfoComponent implements OnInit {
  ...
  removeEmailAddress(index) {
    ...
    APIContactInfoDelete = AppSettings.APIsNameArray.CARRIERSCONTACTINFOR.BROKERDELETE;
     newParams = {
      brokerContactInformationId: this.emailID,
    }
  ...
  removePhoneAddress(index) {
    ...
    APIContactInfoDelete =  AppSettings.APIsNameArray.CARRIERSCONTACTINFOR.BROKERDELETE;
     newParams = {
      brokerContactInformationId: this.phoneID
   }
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
    let APIparams = {
      apiKey: APIContactInfoDelete,
  ...
  getUserProfile() {
    ...
    if(localStorage.getItem('user_type') == 'CARRIER'){
     ...
     APIuserget = AppSettings.APIsNameArray.USER.BROKERGET;
   }
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
  ...
  getCarrierContactInfo() {
    ...
    let uri = null;
    let APIContactInfo
      APIContactInfo = AppSettings.APIsNameArray.CARRIERSCONTACTINFOR.BROKERLIST;
    let newParams = {
    };
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
    let APIparams = {
