// changed the select state input filed lane and terminal and region 
...
export class BrokerLanesRegionsComponent implements OnInit {
  ...
  terminalSubmit({ value, valid }) {
    ...
    if (valid) {
      ...
      if (!cityIdFrom) {
        ...
        let APIparams = {
          apiKey: AppSettings.APIsNameArray.TERMINAL.BROKERADD,
          uri: '',
          postBody: value,
        };
        this.commonService.post(APIparams).subscribe(
  ...
  getTerminalist() {
    ...
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
    let APIparams = {
      apiKey: AppSettings.APIsNameArray.TERMINAL.BROKERLIST,
      uri: uri,
    };
    this.commonService.getList(APIparams).subscribe((ServerRes) => {
      if (ServerRes.success === true) {
  ...
  terminalDelete(index) {
    ...
    let uri = null;
    let newParams = {
      brokerTerminalId: this.terminalListID
    }
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
    let APIparams = {
      apiKey: AppSettings.APIsNameArray.TERMINAL.BROKERDELETE,
      uri: uri,
    };
    this.commonService.delete(APIparams).subscribe(
      (success) => {
