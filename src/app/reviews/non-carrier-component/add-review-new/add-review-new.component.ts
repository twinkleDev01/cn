...
@Component({
  selector: 'app-add-review-new',
  templateUrl: './add-review-new.component.html',
  styleUrls: ['./add-review-new.component.css'],
  providers: [
...
export class AddReviewNewComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.carrierId = this.route.snapshot['queryParams']?.carrierId;
  ...
  submitReview({ value, valid }) {
    ...
    if (valid) {
      ...
      formData.append('relatedToCarrier', value.relatedToCarrier);
      ...
      formData.append('whatWentWell', JSON.stringify(Object.values(this.selectWhatWentWellValue).map(value => value.toString())));
      formData.append('whatWentPoorly', JSON.stringify(Object.values(this.selectWhatWentPoorlyValue).map(value => value.toString())));
  ...
  whatWentPoorlyCheckboxChange(event: MatCheckboxChange, id: any, index: any) {
    const whatWentPoorlyCheckbox = this.addReviewForm.get('whatWentPoorly');
    if (event.checked) {
      this.selectWhatWentPoorlyValue.push(id);
      this.addReviewForm.get('whatWentPoorly').setValue(this.selectWhatWentPoorlyValue)
    ...
    whatWentPoorlyCheckbox.markAsTouched();
    whatWentPoorlyCheckbox.updateValueAndValidity();
  }

  whatWentWellCheckboxChange(event: MatCheckboxChange, id: any, index: any) {
    const whatWentWellCheckbox = this.addReviewForm.get('whatWentWell');

    if (event.checked) {
      this.selectWhatWentWellValue.push(id);
      this.addReviewForm.get('whatWentWell').setValue(this.selectWhatWentWellValue)
    } else {
    ...
    whatWentWellCheckbox.markAsTouched();
    whatWentWellCheckbox.updateValueAndValidity();
  }
  ...
  getUserProfile() {
    let uri = null;
    let newParams = { 'carrierId': this.carrierId };
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
    let APIparams = {
  ...
  selectCarrierDiscovery($event) {
    if ($event.value) {
      this.addReviewForm.get("carrierDiscovery").setValue($event.value);
    }
  }
  ...
  generateReview() {
    ...
    if (this.AIInstructions == 'ai') {
      if (this.addReviewForm.controls?.title.status == 'VALID' && this.addReviewForm.controls?.rating.status == 'VALID' && this.addReviewForm.controls?.tracking.status == 'VALID' && this.addReviewForm.controls?.rateForTimeliness.status == 'VALID' && this.addReviewForm.controls?.rateForCleanliness.status == 'VALID' && this.addReviewForm.controls?.rateForCommunication.status == 'VALID' && this.addReviewForm.controls?.isConsiderNextTime.status == 'VALID' && this.addReviewForm.controls?.considerExpensive.status == 'VALID' && this.addReviewForm.controls?.whatWentPoorly.status == 'VALID'
        && this.addReviewForm.controls?.whatWentWell.status == 'VALID' && this.selectCarrierData.companyName) {
        this.loading = true;
        requestBody = {
          review: {
            ...
            "rateForTimeliness": this.addReviewForm.controls?.rateForTimeliness.value,
            "rateForCleanliness": this.addReviewForm.controls?.rateForCleanliness.value,
            ...
            "whatWentPoorly": this.addReviewForm.controls?.whatWentPoorly.value,
            "whatWentWell": this.addReviewForm.controls?.whatWentWell.value
          },
          "type": 'ai',
        }
      }
