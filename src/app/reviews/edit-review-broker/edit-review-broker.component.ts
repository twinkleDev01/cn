...
@Component({
  selector: 'edit-review-broker',
  templateUrl: './edit-review-broker.component.html',
  styleUrls: ['./edit-review-broker.component.css'],
  providers: [
...
export class EditReviewBroker {
  ...
  getReviewData() {
    ...
    APIparams = {
      apiKey: AppSettings.APIsNameArray.REVIEW.BROKERGETREVIEW,
      uri: uri,
    };
    this.commonService.getList(APIparams).subscribe((ServerRes) => {
      if (ServerRes.success === true) {
  ...
  getValueForm() {
    if (this.getReview) {
      ...
      this.relatedToBroker = this.getReview?.relatedToBroker;
      ...
      this.editReviewForm = this.formBuilder.group({
        ...
        recommendation: [this.getReview.recommendation ? this.getReview?.recommendation : '', [Validators.required]],
        ...
        relatedToBroker: [this.getReview?.relatedToBroker ? this.getReview?.relatedToBroker : false, Validators.required],
        ...
        brokerDiscovery: [this.getReview?.carrierDiscoveryType ? this.getReview?.carrierDiscoveryType?.carrierDiscoveryTypeId : null,],
  ...
  submitReview({ value, valid }) {
    ...
    this.editReviewForm?.get('relatedToBroker').setValue(this.getReview?.relatedToBroker);
    ...
    if (valid) {
      ...
      formData.append('relatedToBroker', this.relatedToBroker);
      ...
      let APIparams = {
        ...
        apiKey: AppSettings.APIsNameArray.REVIEW.BROKEREDITUPDATE,
        uri: '',
        postBody: formData,
      };
  ...
  getUserProfile() {
    let uri = null;
    let newParams = { 'brokerId': localStorage.getItem('carrierEditId') };
    if (newParams) uri = this.commonService.getAPIUriFromParams(newParams);
    let APIparams = {
  ...
  isValueSelected(type): boolean {
    ...
    else if (type == 'relatedToBroker') {
      return this.relatedToBroker !== undefined && this.relatedToBroker !== null;
    }
  ...
  generateReview() {
    ...
    if (this.AIInstructions == 'ai') {
      if (this.editReviewForm.controls?.title.status == 'VALID' && this.editReviewForm.controls?.rating.status == 'VALID' && this.editReviewForm.controls?.tracking.status == 'VALID' && this.editReviewForm.controls?.recommendation.status == 'VALID'   && this.editReviewForm.controls?.rateForCommunication.status == 'VALID' && this.editReviewForm.controls?.isConsiderNextTime.status == 'VALID' && this.editReviewForm.controls?.considerExpensive.status == 'VALID' 
        ...
        requestBody = {
          review: {
            ...
            "recommendation": this.editReviewForm.controls?.recommendation.value,
